...
def update_material_information(ifc_element, new_material_info):
    # Extrahiere das vorhandene Material des IFC-Elements
    ifc_material = ifcopenshell.util.element.get_material(ifc_element)

    # Überprüfe, ob das Material vorhanden ist
    if ifc_material:
        # Annahme: new_material_info ist ein Dictionary mit Materialinformationen
        # Beispiel: new_material_info = {"Material": "NeuesMaterial", "LayerThickness": "2 cm"}

        # Überprüfe, ob die Materialinformationen gültig sind
        if "Material" in new_material_info:
            # Setze den neuen Materialnamen
            ifc_material.Name = new_material_info["Material"]

            # Füge hier weitere Aktualisierungen hinzu, je nachdem, wie deine Materialinformationen strukturiert sind.
            # Beispiel: Aktualisierung der LayerThickness
            if "LayerThickness" in new_material_info:
                # Annahme: LayerThickness ist ein Attribut des Materials
                ifc_material.LayerThickness = new_material_info["LayerThickness"]

        # Füge hier weitere Aktualisierungen hinzu, je nach Bedarf und Struktur deiner Materialinformationen.

# Weitere Helper-Funktionen können hier folgen
